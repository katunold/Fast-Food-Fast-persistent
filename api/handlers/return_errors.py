...
class ReturnError:
    ...
    def invalid_data_type(data_type, field):
        if data_type == "int":
            response_object = {
                "status": "fail",
                "message": "Only {} data type supported for {}".format(data_type, field),
                "data": False}
        else:
            response_object = {
                "status": "fail",
    ...
    def invalid_password():
        return jsonify({
            "status": "fail",
            "message": "Password is wrong. It should be at-least 6 characters"
    ...
    def invalid_email():
        ...
        return jsonify({
            "status": "fail",
            "message": "User email {0} is wrong, It should be "
    ...
    def invalid_contact():
        return jsonify({
            "status": "fail",
            "message": "Contact {0} is wrong. should be in"
    ...
    def item_not_on_the_menu(order_item):
        response_object = {
            'status': 'fail',
            'message': 'Sorry, Order item {} not on the menu'.format(order_item)
        }
    ...
    def invalid_name():
        return jsonify({
            "status": "fail",
            "message": "A name should consist of only alphabetic characters",
            "data": False
        }), 400
    ...
    def invalid_user_type():
        ...
        return jsonify({
            "status": "fail",
            "message": "User type {0} does not exist ".format(req['user_type']),
            "data": req
        }), 400
    ...
    def order_status_not_found(order_status):
        return jsonify({
            "status": "fail",
            "message": "Order status {} not found".format(order_status),
            "data": False}), 404
